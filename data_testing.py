...
#outcome metrics to keep track of
accuracy_results = []
f1_results = []

for stock in stock_symbols:

    print(f"\nTraining with stock: {stock}")
    ...
    #keep track of testing for plotting afterwards
    ...
    for features in feature_combinations:
        ...
        print(f"\nF1 Score for {stock}: {f1:.3f}")
        print(f"Accuracy for {stock}: {accuracy:.3f}")
...
#get average stats
avg_acc = sum(accuracy_results)/4
avg_f1 = sum(f1)/4

print(f"\nAverage Accuracy: {sum(accuracy_results)/4:.3f}")
print(f"Average F1-score: {sum(f1)/4:.3f}")
#plot predictions vs actual closing prices
...
plt.plot(test_dates_pd, actual_close, label='Actual Close', marker='o', linestyle='-', color='blue')

#plot predicted closing prices
plt.plot(test_dates_pd, close_pred, label='Predicted Close', marker='x', linestyle='--', color='red')

# add labels and title
plt.xlabel('Date')
plt.ylabel('Price ($)')
plt.title(f'Stock Price Prediction using {", ".join(features)}')
plt.legend()
...
#display accuracy metrics on the plot
plt.figtext(0.15, 0.85, f'Accuracy: {accuracy:.3f}', fontsize=12, bbox={"facecolor":"white", "alpha":0.8, "pad":5})
plt.figtext(0.15, 0.80, f'F1 Score: {f1:.3f}', fontsize=12, bbox={"facecolor":"white", "alpha":0.8, "pad":5})
